...
def build_summary_csv():
    ...
    writer.writerow([])
    writer.writerow(["Agent vs Baseline"])
    writer.writerow(["Approach", "Amount recovered (Rs)", "Recovery rate"])
    writer.writerow(["Agent", core["amount_recovered"], _pct(core["recovery_rate"])])
    writer.writerow(["Naive baseline", base["amount_recovered"], _pct(base["recovery_rate"])])
    writer.writerow([])
