...
# Deliberately spoofed webhooks: these customer_ids get a corrupted signature to
# simulate tampered/forged events. They must fail verification and be rejected before
# entering the agent pipeline (proves the security check works during the demo).
SPOOFED_CUSTOMER_IDS = ("CUST1007", "CUST1042", "CUST1099")
...
def build_records(rng, total=TOTAL):
    ...
    # Sign every event with an HMAC-SHA256 signature over its canonical payload.
    # This happens after all RNG draws so it never affects reproducibility. A small
    # deterministic set of events is then given a corrupted signature to simulate
    # spoofed webhooks that must be rejected at ingestion.
    for record in records:
        sig = webhook_security.sign_payload(record)
        if record["customer_id"] in SPOOFED_CUSTOMER_IDS:
            # Tamper with the signature so verify_signature() will fail for these.
            sig = "deadbeef" + sig[8:]
        record["webhook_signature"] = sig
    return records
